module temporal_fusion_threat_assessment #(
    ...
    input wire [NUM_MFCCS*DATA_WIDTH-1:0] audio_mfcc_flat,
    ...
    output wire signed [DATA_WIDTH-1:0] debug_correlation_score,
    output wire signed [DATA_WIDTH-1:0] debug_anomaly_score,
    output wire signed [DATA_WIDTH-1:0] debug_pattern_score
);
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (motion_direction_changes > 16'h3000) harassment_match_count = harassment_match_count + 10;
            if (motion_total_accel       > 16'h5000) harassment_match_count = harassment_match_count + 15;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (fall_state_tracker)
                FALL_NORMAL:    if (motion_total_accel < 16'h0800) begin fall_state_tracker <= FALL_FREE_FALL; fall_score <= 8'd30; end
                FALL_FREE_FALL: begin
                    if      (motion_jerk > 16'h7800)         begin fall_state_tracker <= FALL_IMPACT;  fall_score <= fall_score + 40; end
                    else if (motion_total_accel > 16'h2000)  begin fall_state_tracker <= FALL_NORMAL;  fall_score <= 8'd0; end
                end
                FALL_IMPACT: begin
                    if (motion_total_accel < 16'h1000 && motion_variance < 16'h0800)
                                                             begin fall_state_tracker <= FALL_STILLNESS; fall_score <= fall_score + 30; end
                    else                                     begin fall_state_tracker <= FALL_NORMAL;    fall_score <= 8'd0; end
                end
                FALL_STILLNESS: fall_state_tracker <= FALL_NORMAL;
            endcase
            ...
            if (!baseline_learned) begin
                if (audio_features_valid) begin
                    baseline_audio_energy_mean <=
                        ((baseline_audio_energy_mean * baseline_sample_count) + audio_energy)
                ...
                if (motion_features_valid) begin
                    baseline_motion_accel_mean <=
                        ((baseline_motion_accel_mean * baseline_sample_count) + motion_total_accel)
                        / (baseline_sample_count + 1);
                end
            end
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (assault_score     > pattern_max_score) begin pattern_max_score = assault_score;     matched_category = CATEGORY_ASSAULT; end
            if (fall_score        > pattern_max_score) begin pattern_max_score = fall_score;        matched_category = CATEGORY_FALL;    end
            if (chase_score       > pattern_max_score) begin pattern_max_score = chase_score;       matched_category = CATEGORY_CHASE;   end
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if      (threat_confidence >= config_alert_threshold) begin alert_level <= ALERT_IMMEDIATE; npu_interrupt <= 1'b1; end
            else if (threat_confidence >= config_npu_threshold)   begin alert_level <= ALERT_WAKE_NPU;  npu_interrupt <= 1'b1; end
            else if (threat_confidence >= config_log_threshold)   begin alert_level <= ALERT_LOG;       npu_interrupt <= 1'b0; end
            else                                                   begin alert_level <= ALERT_NONE;      npu_interrupt <= 1'b0; end
        end
    end
